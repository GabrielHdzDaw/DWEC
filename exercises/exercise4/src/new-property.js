...
mainPhotoInput.addEventListener("change", async (event) => {
  ...
  if (!file.type.startsWith("image")) {
    ...
    mainPhotoInput.setCustomValidity(
      "El tamaño de la imagen no puede ser superior a 200KB"
    );
  } else {
    mainPhotoInput.setCustomValidity("");
  }
...
const validateForm = (formData) => {
  ...
  if (!title.trim()) {
    ...
    titleInput.setCustomValidity(
      "Debe empezar por mayúscula y solo contener letras, números o espacios"
    );
    valid = false;
  } else {
    titleInput.setCustomValidity("");
  ...
  if (
    !numBaths ||
    isNaN(parseInt(numBaths)) ||
    parseInt(numBaths) < 1 ||
    parseInt(numBaths) > 20
  ) {
  ...
  if (!mainPhoto || !mainPhoto.type.startsWith("image")) {
    ...
    mainPhotoInput.setCustomValidity(
      "El tamaño de la imagen no debe superar los 200KB"
    );
    valid = false;
  } else {
    mainPhotoInput.setCustomValidity("");
